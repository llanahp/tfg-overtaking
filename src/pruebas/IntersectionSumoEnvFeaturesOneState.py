...
class CustomEnv(Env):
	...
	# Return the coordinates of the vehicles in range
	def _get_vehicle_in_range(self):
		adversaries_yaw = []
		adversaries_y = []
		adversaries_x = []
		adversaries_v = []
		...
		return adversaries_x, adversaries_y, adversaries_v, adversaries_yaw
	...
	def _get_vehicle_intirior(self):
		...
		resul = []
		adversaries_x, adversaries_y, adversaries_v, adversaries_yaw = self._get_vehicle_in_range()
		x_ego, y_ego = self._get_coords_ego()
	...
	def _addCar(self):
		...
		routeInside = ["rt1", "rt2", "rt3", "rt4", "rt5", "rt6", "rt7", "rt8"]
		...
		if self.egoCarID in traci.vehicle.getIDList():
			if traci.vehicle.getLaneID(self.egoCarID).startswith("-") == False:
				...
				if self.inside_cars == []:
					...
					# si hay coches en el interior pero estan muy lejos
					if self.inside_cars[1] < 0.8:
						return 0
		return prev_vel