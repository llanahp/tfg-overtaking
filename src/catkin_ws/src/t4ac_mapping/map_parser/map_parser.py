...
from .map_classes import T4ac_Header, T4ac_Road, T4ac_Junction, T4ac_Geometry
from .map_classes import T4ac_Elevation, T4ac_Lane, T4ac_LaneSection
from .map_classes import T4ac_Object, T4ac_Signal, T4ac_SignalReference, T4ac_Validity
from .map_classes import T4ac_LaneOffset, T4ac_LaneWidth, T4ac_RoadMark
from .map_classes import T4ac_VectorLane, T4ac_CornerLocal, T4ac_Junction, T4ac_Connection
from .map_classes import T4ac_LaneLinkJunction, T4ac_ControllerJunction, T4ac_VectorJunction
from .landmark import T4ac_Landmark, T4ac_LandmarkRoad, T4ac_LandmarkPose
        ...
        for road in roads:
            if len(road.signals.signal) > 0:            
                for signal in road.signals.signal:
                    ...
                    affecting_road.pose = T4ac_LandmarkPose(signal_s, signal_t, signal_zOffset, signal_hOffset)
            ...
            if len(road.objects) > 0:
                for object in road.objects:
                    ...
                    affecting_road = T4ac_LandmarkRoad()
                    affecting_road.road_id = road.id
                    affecting_road.pose = T4ac_LandmarkPose(object_s, object_t, object_zOffset, 0.0)
                    ...
                    # Por el momento, CARLA and T4AC maps no tiene validities para los objectos -> no sabemos a cual afecta
                    ...
                    for i_landmark, landmark in enumerate(landmarks):                        
                        if (landmark.id == int(signalReference.id)):
                            ...
                            if not road_in_affecting_roads:
                                ...
                                affecting_road.pose = T4ac_LandmarkPose(signal_s, signal_t, 0, 0)
