...
def createModel(args, policy, policy_kwargs, env, log_dir, model_dir):
	if args.model == "PPO": 
		if args.pretrained == "no":
			...
			print(os.path.join(model_dir, '{}_{}_'.format(args.env, args.model)) + args.pretrained)
			model = PPO.load(os.path.join(model_dir, '{}_{}_'.format(args.env, args.model)+args.pretrained), env=env)

	elif args.model == "SAC":
		if args.pretrained == "no":
			model = SAC(policy, env, verbose = 1, policy_kwargs=policy_kwargs, tensorboard_log=log_dir)
		else:
			model = SAC.load(os.path.join(model_dir, '{}_{}_'.format(args.env, args.model)), env=env)
