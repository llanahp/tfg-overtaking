...
class CustomCombinedExtractorMultiState(BaseFeaturesExtractor):
        ...
        for key, subspace in observation_space.spaces.items():
            if key == "adversaries":
                ...
                extractors[key] = nn.Sequential(
                    nn.Conv1d(in_channels=subspace.shape[0], out_channels=1, kernel_size=1, stride=1, padding=0),
                    nn.ReLU()
                )
    ...
    def forward(self, observations) -> th.Tensor:
        ...
        for key, extractor in self.extractors.items():
            ...
            print((extractor(observations[key][0])).size())
            # print("Tensor ", key, " :", extractor(observations[key][0]))
